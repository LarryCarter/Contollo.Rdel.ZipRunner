...
namespace Contollo.Rdel.ZipRunner.AI.Engine
{
    internal sealed class ContextAssemblyEngine
    {
        ...
        {
            Register(new ProtocolProvider()); Register(new AiContractProvider()); Register(new ProjectProvider());
            Register(new ProjectDocsProvider()); Register(new MemoryProvider()); Register(new DecisionsProvider());
            Register(new ActiveDocumentProvider()); Register(new GitStatusProvider()); Register(new OutputPlaceholderProvider());
            Register(new UserRequestProvider());
        }
        ...
        public string Assemble(AiSessionContextKind kind, DTE2 dte)
        {
            ...
            {
                Kind = kind, Dte = dte, SolutionRoot = solutionRoot,
                SelectedProjectRoot = RdelSolutionLocator.GetSelectedProjectRoot(dte),
                RepositoryName = string.IsNullOrWhiteSpace(solutionRoot) ? "Unknown" : Path.GetFileName(solutionRoot.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar))
            };
            ...
            foreach (string providerName in template.Providers)
            {
                try
                {
                    ...
                    sections.Add(providers.TryGetValue(providerName, out provider)
                        ? provider.Build(request)
                        : new ContextSection("Missing Provider: " + providerName, "Not registered."));
                }
